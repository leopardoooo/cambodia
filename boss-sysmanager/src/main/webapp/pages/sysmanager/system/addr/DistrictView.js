...
//	            autoWidth: true,
	        ...
	        obarCfg: {
	            ...
	            btns: [{
	                ...
	                handler : function(n){
	                	new DistrictWin('leveladd',n).show();
	                }
	            },{
	                id: 'edit',
...
DistrictTreeCombo = Ext.extend(Ext.ux.TreeCombo,{
	listeners:{
			...
			this.itemForm.add(new AddressDistrictTreeCombo({
				...
				listeners:{
					...
					'select': function(combo){
						Ext.getCmp('districtItemId').setValue(combo.getValue());
					}
				}
			}));
			
			if( this.level == 3 && this.type =='edit'){
				Ext.getCmp('districtCfgItemId').setValue(this.node.attributes.others.district_name);
				Ext.getCmp('districtItemId').setValue(this.node.attributes.others.district_id);
			}
		}
	...
	doSave : function(){
		...
		if(this.itemForm.getForm().isValid()){
			Confirm(langUtils.sys('AddressNodeManage.msg.confirmSave'), this ,function(){
				...
				if((this.level == 2 && this.type =='add') || this.level == 3){
					params['addrDto.district_id'] = Ext.getCmp('districtItemId').getValue();
				}
