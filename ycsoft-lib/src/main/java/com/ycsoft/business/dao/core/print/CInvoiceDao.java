...
//				+ " and pf.fee_sn=?)";
	...
	public List<CInvoiceDto> queryInvoiceByCustId(String custId,String countyId) throws JDBCException {
		String sql = " select distinct 'A' invoice_mode, i.doc_sn, i.optr_id,i.invoice_code, i.invoice_id, ri.status,ri.finance_status, i.amount,  print_date, i.invoice_book_id,"
				+ " ri.use_time,ri.invoice_type doc_type,f.invoice_id fee_invoice_id,f.invoice_code fee_invoice_code,f.status fee_invoice_status,f.done_code fee_done_code"
				...
				+ " from c_invoice i ,c_doc d,r_invoice ri,r_invoice_feelist f"
				+ " where i.doc_sn=d.doc_sn AND ri.status=:STATUS"
				+ " AND  i.invoice_code=ri.invoice_code and i.invoice_id=ri.invoice_id"
				+ " and ri.invoice_id=f.feelist_id(+) and ri.invoice_code=f.feelist_code(+)"
				...
				+ " FROM c_fee c,r_invoice ri,r_invoice_feelist f"
				+ " WHERE c.invoice_mode='M' AND c.CUST_ID =:CUSTID AND c.COUNTY_ID =:COUNTYID"
				+ " AND ri.invoice_id=c.invoice_id AND ri.invoice_code=c.invoice_code"
				+ " and ri.invoice_id=f.feelist_id(+) and ri.invoice_code=f.feelist_code(+)"
				+ " and c.status=:PAY and c.busi_code <> :UNITPAY"
				+ " GROUP BY c.OPTR_ID,c.INVOICE_CODE, c.INVOICE_ID,ri.status, RI.FINANCE_STATUS,c.INVOICE_BOOK_ID,ri.use_time,ri.invoice_type,f.invoice_id,f.invoice_code,f.status,f.done_code"
				+ " UNION ALL"
				+ " SELECT 'M' invoice_mode,'',c.OPTR_ID,c.INVOICE_CODE,c.INVOICE_ID,ri.status,RI.FINANCE_STATUS,SUM(c.real_pay) AMOUNT,"
				+ " NULL PRINT_DATE,c.INVOICE_BOOK_ID,ri.use_time,ri.invoice_type doc_type,f.invoice_id fee_invoice_id,f.invoice_code fee_invoice_code,f.status fee_invoice_status,f.done_code fee_done_code"
				+ " ,(select max(cf.create_time) from c_fee cf where cf.cust_id=:CUSTID and cf.county_id=:COUNTYID and cf.invoice_id=c.invoice_id and cf.invoice_code=c.invoice_code) fee_create_time"
				+ " FROM c_fee c, r_invoice ri,c_done_code_detail cd,r_invoice_feelist f"
				+ " where c.create_done_code=cd.done_code AND ri.invoice_id = c.invoice_id AND ri.invoice_code = c.invoice_code"
				+ " and ri.invoice_id=f.feelist_id(+) and ri.invoice_code=f.feelist_code(+)"
				+ " and ri.invoice_mode='M' AND c.COUNTY_ID = :COUNTYID and c.status = :PAY and c.busi_code=:UNITPAY and cd.cust_id=:CUSTID "
				+ " GROUP BY   c.OPTR_ID,c.INVOICE_CODE, c.INVOICE_ID,ri.status, RI.FINANCE_STATUS,c.INVOICE_BOOK_ID,ri.use_time,ri.invoice_type,f.invoice_id,f.invoice_code,f.status,f.done_code ORDER BY use_time DESC";
