...
public class CProdOrderDao extends BaseEntityDao<CProdOrder> {
	...
	public CProdOrderDto queryCProdOrderDtoByKey(String order_sn) throws JDBCException{
		String sql=StringHelper.append(" select case when cu.user_id is null then null when   cu.user_type in ('OTT_MOBILE','BAND') then cu.login_name else nvl(cu.stb_id,'unknow') end user_name ,nvl(d.disct_name,ppt.tariff_name) tariff_name,p.prod_name, o.* ",
