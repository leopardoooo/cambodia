...
public class PSpkgOpenuserDao extends BaseEntityDao<PSpkgOpenuser> {
	...
	public void updateOpenUserDoneCode(String spkgSn, Integer doneCode) throws Exception {
		String sql = "update P_SPKG_OPENUSER set use_done_code=?, status=? where sp_id=(select sp_id from p_spkg where spkg_sn=?)";
		this.executeUpdate(sql, doneCode, StatusConstants.USE, spkgSn);
		
		sql = "update P_SPKG_OPENBUSIFEE set use_done_code=?, status=? where sp_id=(select sp_id from p_spkg where spkg_sn=?)";
		this.executeUpdate(sql, doneCode, StatusConstants.USE, spkgSn);
	}
	...
	public List<PSpkgOpenuser> querySpkgUser(String spkgSn) throws Exception {
		String sql = "select so.*, bm.buy_mode_name, f.fee_name"
				...
				+ " and s.spkg_sn = ? and f.fee_type(+)=? and so.status=?";
		return this.createQuery(sql, spkgSn, SystemConstants.FEE_TYPE_DEVICE, StatusConstants.IDLE).list();
	}
